...
<script async="async">
	 ...
	 chat.sendMessage = function () {
		...
		default:
			...
			if(msg > 0 && msg <= <%=numOfItems%> && <%=numOfItems%>!=-1){
				...
				$.get('gameservlet', { action: "buyFromStore", username: '<%=username%>', itemList: itemList[msg]}, 
	...
	function displayStore(){
		...
		var k =1;
		itemList = [];
		Console.log(font("blue")+"Type the number of the item you want to buy");
		$.get("gameservlet", {action: "getItemsDB", username: username},
			function(responseJson){	
			$.each(responseJson, function(index,value){
				Console.log("");
				Console.log("<b>"+font("#33aaaa")+k+":");
				k++;
					$.each(value, function(key, subvalue){
						
						Console.log("<b>"+font("blue")+key+": "+subvalue);
						if(key=="Item")
							itemList.push(subvalue);
					});
		...
		setTimeout(function(){ 
			mode="store";//Console.log(itemList[0]); 
		} , 100);		
