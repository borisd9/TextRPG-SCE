...
public class GameDB extends DbConnectionAPI {
	...
	public void addPlayer(String username, String charName) {
		...
		try {
			if (rs.next()) {
				...
				String query2 = "INSERT INTO players VALUES ('" + username
						+ "','Home','" + charName + "',1,'" + attack + "','"
						+ defense + "','" + speed + "','" + hp
						+ "',0,1000,0,'Starter Kit',0,0)";
	...
	public void updatefight(String username, int exp, int coins, int flag) {
		...
		int intcoins = 0;
		if (flag == 1) {
			String query = "UPDATE players SET exp=exp+" + exp
					+ ",money=money+" + coins + " WHERE username='" + username
					+ "'";
			...
			if (intcheck - exp >= 0 && intcoins - coins >= 0) {
				String query = "UPDATE players SET exp=exp-" + exp
						+ ",money=money-" + coins + " WHERE username='"
						+ username + "'";
				modifyDatabase(query);
			} else if (intcoins - coins >= 0) {
				String query = "UPDATE players SET money=money-" + coins
	...
	public void updatePlayerMoney(String playerName, String price) {
		...
		try {
			if (rs.next()) {
				...
				String qr = "UPDATE players SET money='" + money
	 ...
	 * 
	 * @param playerName
	 *            , item updates player item field with itemName that he buy
	 */
	public void updatePlayerItem(String playerName, String item) {
		...
		try {
			if (rs.next()) {
				String qr = "UPDATE players SET item='" + item
						+ "'WHERE username='" + playerName + "'";
	...
	public int getdefense(String username) {
		String defense = "SELECT defense FROM players WHERE username='"
				+ username + "'";
	 ...
	 * @param exp
	 */
	public void updateBattleResults(String winner, String loser, float exp,
			String levelup) {
		String query = "UPDATE players SET loses=loses+1 WHERE username='"
				+ loser + "'";
		modifyDatabase(query);
		...
		if (levelup.equals("true")) {
			subquery = "exp=" + exp + ",level=level+1";
			subquery += "attack=attack" + rnd.nextInt(8);
			subquery += "defense=defense" + rnd.nextInt(8);
			subquery += "speed=speed" + rnd.nextInt(8);
			subquery += "hp=hp" + rnd.nextInt(8);
		} else
			subquery = "exp=exp+" + exp;

		query = "UPDATE players SET wins=wins+1," + subquery
	...
	public ResultSet getCharAttacks(String character) {
		String query = "SELECT atk1,atk2 FROM characters WHERE name='"
				+ character + "'";
		ResultSet rs = readFromDatabase(query);
		return rs;
	}
